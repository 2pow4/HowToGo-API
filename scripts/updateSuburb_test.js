...
connect(dbUsername, dbPwd, dbIP, dbPort, dbName)
  ...
  //    해당 도시코드를 통하여 정류소 조회
  .then(async cityList => {
    const locationFetchValues = []
    const _locationFetchValues = cityList.map((city) => {
      const { cityCode } = city
    ...
    locationFetchValues.push(_locationFetchValues[23])
    locationFetchValues.push(_locationFetchValues[24])
    locationFetchValues.push(_locationFetchValues[25])
    locationFetchValues.push(_locationFetchValues[26])
